...
function GameBoard() {
    ...
    //Basic elements - Define the Combo to check win or loose
    ...
    //(return the 2D array board, not the object board)
    const getBoard = () => board;

    //Return Function: display the board
    const displayBoard = () => {
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 3; j++) {
                const gridItem = document.querySelector(`#board${i}${j}`);
...
function GameController (
    ...
    //Inner Function: get empty cell array
    //(It's important to use a function to get the emptyCellsArray istead of name the variable "emptyCellsArray", function can update the variable everytime call it)
    const getEmptyCellsArray = () => 
        board.getBoard().flat().filter(el => el.getValue() === 0); 
    ...
    const getPseudoAIMove = () => { 
        ...
        //1. CallBack Method to find the Combo that the AI with last move to win, like [2, 2, 0]
        const if2_2_0 = (array3Els) => 
            array3Els.filter(el => el.getValue() === 2).length === 2 &&
            array3Els.filter(el => el.getValue() === 0).length === 1;

        //2. CallBack Method to find the Combo that the Gamer with last move to win, like [1,1,0]
        ...
        //3. Check win combo and block combo first, if above two combos don't exist, find ramdom empty cell and place into it. 
   
        const emptyCellsArray = getEmptyCellsArray();
        ...
        //(It's a Cell object being returned, like 'board[1][2]', which equal to a Cell object, Cell.row=1, Cell.column=2)
        return theMovePlace;
    }
    ...
    const returnWinnerToken = () => {
        //callback method: find an array has 3 same elements)
        const findWinCombos = (ifArray3SameELes) => {
            const ifAll1 = ifArray3SameELes.every(element => element.getValue() === 1);
            const ifAll2 = ifArray3SameELes.every(element => element.getValue() === 2);
            if (ifAll1 || ifAll2) {
                return true;
            } else {
                return false;
    ...
    //Inner Function: initial game
    // (clear the resultDisplay, clear the grid innerHTML, reset the board token to 0)
    const initialGame = () => {
    ...
    const checkWin = () => {
        ...
        if (winnerToken === 1) {
            ...
            //(add a onetime eventListener which can clear the board and the result display, so after if there is an winner, if you click the board, the board will initial)
            //(It's important to use 'setTimeout()' instead of set the eventLisntener directally, because the browser will add the eventListener "before" the click, so when click grid, it will call "game.playOneRound" and this at the same time. So at the last move to vin, strange things will happen)
            setTimeout(() => {
    ...
    const ifBoardIsFull = () => {
        if (getEmptyCellsArray().length === 0) {
            setTimeout(() => {
                boardDisplay.addEventListener('click', () => initialGame(), {once: true})
            }, 0);
            return true;
    ...
    const playOneRound = (row, column) => {
        ...
        //(this statement will run 'checkWin()' at the same time and get the return of it, that's important.)
        if (checkWin()) return;
